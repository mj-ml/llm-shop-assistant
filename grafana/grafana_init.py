...
def create_api_key():
    ...
    if response.status_code == 200:
        ...
        keys_response = requests.get(
            f"{GRAFANA_URL}/api/auth/keys", auth=auth
        )
            ...
            for key in keys_response.json():
                if key["name"] == "ProgrammaticKey":
                    ...
                    delete_response = requests.delete(
                        f"{GRAFANA_URL}/api/auth/keys/{key['id']}",
                        auth=auth,
                    )
...
def create_or_update_datasource(api_key):
    ...
    if response.status_code == 200:
        ...
        print(
            f"Updating existing datasource with id: {datasource_id}"
        )
    ...
    if response.status_code in [200, 201]:
        ...
        return response.json().get("datasource", {}).get(
            "uid"
        ) or response.json().get("uid")
    else:
        print(
            f"Failed to create or update datasource: {response.text}"
        )
        return None
...
def create_dashboard(api_key, datasource_uid):
    ...
    dashboard_file = "data/grafana_dash.json"
    ...
    print(
        f"Updated datasource UID for {panels_updated} panels/targets."
    )
